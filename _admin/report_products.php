                            ...
                            <div class="table-responsive p-3">
                                <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                                    ...
                                    <tbody>
                                        ...
                                        $total_quantity = 0;
                                        $total_price = 0;
                                        $query_product = mysqli_query($con, "SELECT product_id, product_name, product_type_name, product_quantity, product_price FROM products, product_types WHERE products.product_type_id=product_types.product_type_id;");
                                        while ($row = mysqli_fetch_array($query_product)) {
                                            $GLOBALS['total_quantity'] += $row['product_quantity'];
                                            $GLOBALS['total_price'] += $row['product_price'] * $row['product_quantity'];
                                            ?>
                                            <tr>
                                                <td><?php echo htmlentities($row['product_id']); ?></td>
                                                <td><?php echo htmlentities($row['product_name']); ?></td>
                                                <td><?php echo htmlentities($row['product_type_name']); ?></td>
                                                <td><?php echo htmlentities($row['product_quantity']); ?></td>
